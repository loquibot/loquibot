...
public class SpamProtectionSettings {
    ...
    public static JPanel createPanel(){
        ...
        settingsPage.addConfigCheckbox("$LINK_SPAM$", "$LINK_SPAM_DESC$", "linkFilterEnabled", SpamProtectionSettings::openLinkFilterSettings);
        settingsPage.addConfigCheckbox("$GIBBERISH_SPAM$", "$GIBBERISH_SPAM_DESC$", "gibberishFilterEnabled", SpamProtectionSettings::openGibberishFilterSettings);
    ...
    public static class TextInput extends JPanel {
        ...
        TextInput(String text, String description, boolean isPercentage, int numberLimit, String setting, String defaultInput, boolean allowDecimal){
            ...
            textArea.getDocument().addDocumentListener(new DocumentListener() {
                ...
                public void changed() {
                    if(textArea.getText().equalsIgnoreCase("")){
                        ...
                        if (isPercentage) Settings.writeSettings(setting, String.valueOf(Double.parseDouble(textArea.getText()) / 100));
                        else Settings.writeSettings(setting, textArea.getText());
                    }
                }
            });
            ...
            if(isPercentage){
                if (Settings.getSettings(setting).exists()) {
                    if (!allowDecimal) textArea.setText(String.valueOf((int) (Settings.getSettings(setting).asDouble() * 100)));
                    else textArea.setText(String.valueOf(Settings.getSettings(setting).asDouble() * 100));
                }
                else {
